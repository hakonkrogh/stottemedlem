...
- The page presents the organization's **visual identity** when uploaded, in
  ...
  public page of a member's [card](member-card.md), and the top of every
  screen of the [back office](back-office.md), so what an administrator sees
  above their work is what a supporter sees.
...
- The pages are usually **arrived at from another app** (a link in an email, a
  chat message, or the hand-back from Vipps) and open however the visitor's
  browser opens them, with generous air at both ends so they read even when a
  phone browser hands the page over with its top tucked under the status bar
  ([Opening a page](opening-a-page.md)).
