...
const filters = [];
if (opts.service) filters.push(leaf("$metadata.service", "eq", opts.service, "string"));
...
for (const raw of opts.filters) {
  ...
  const numeric =
    /^-?\d+(\.\d+)?$/.test(rawVal) &&
    !["includes", "not_includes", "starts_with", "ends_with"].includes(op);
...
function needleParam() {
  return opts.search ? { value: opts.search, isRegex: opts.regex, matchCase: false } : undefined;
}
...
// ── rendering ──────────────────────────────────────────────────────────────
const trunc = (s, n) => (!opts.full && s.length > n ? `${s.slice(0, n)} …[truncated]` : s);
...
function renderEvent(e) {
  ...
  const src = typeof e.source === "string" ? e.source : JSON.stringify(e.source ?? "");
  if (src && src !== msg && src !== "{}" && !echoes) line += `  src=${trunc(src, 400)}`;
...
if (command === "events" || command === "invocations") {
  ...
  if (view === "events") {
    ...
    if (total > ev.length && ev.length)
      console.log(`-- more: rerun with --cursor '${ev[ev.length - 1].$metadata?.id}'`);
  } else {
  ...
  for (const r of rows) {
    const label = r.groups?.map((g) => g.value).join(" | ") ?? r.key ?? "(all)";
