...
- `apps/backoffice/` — Astro 7 SSR on ONE Cloudflare Worker (`src/worker.ts`:
  ...
  **A SECOND form on such a page must `return` before the profile handler
  runs** (bit 2026-08-31 adding the databehandleravtale accept button to
  `innstillinger.astro`): that handler reads EVERY profile field straight off
  the `FormData`, so a one-field POST falling through blanks orgnr/contact/name
  on the public page. Branch on a marker field at the very top of the POST
  block and answer with a 303 back to the page — never let it fall through.
  **Adding a column to `organizations`/`supporting_members` breaks two fixture
  ...
  **Back-office screens cannot be driven locally** — the seeded org belongs to
  ...
  unit-test it there (`packages/db/src/index.test.ts` is a plain vitest suite of
  pure functions, no D1 harness); `hasAcceptedDpa` is the worked example.
  ...
  **Good news is `info` and reaches NO ONE, with one exception.** Local dev
...
- `packages/db/` — `@stottemedlem/db` (added 2026-07-28, scaffolding step 4):
  ...
  **The notice rule silently swallowed a fee RISE that the member asked for**
  ...
  So move a member from a 200 kr tier to a 500 kr one and — with no notice row
  for the new tier — the fallback is their last payment, and the renewal
  charges **200**. The upgrade does not happen, and nothing says so until a
  renewal runs. DOWNGRADES are fine for free: `min()` lets a price cut through
  at once, deliberately ("being charged less than announced is not the kind of
  ...
  **Why "pay the difference now" is not a small feature:** such a charge lands
  ...
  difference first; taking tier changes at the next renewal (which is already
  the tier spec's rule — changes never retroactively alter what was paid)
  avoids the collision, the proration maths and the refund window together.
...
- Tooling gotchas: `astro check` emits false ts(6133) "declared but never read"
  ...
  **Reformatting a long-unformatted config is safe but noisy** (`wrangler.jsonc`
  ...
  survives (`grep -c '^\s*//'`).
- **Money strings hold an invisible NO-BREAK SPACE, so exact-match edits miss
  them** (hit 2026-09-09 rewriting a receipt subject line). `kr()` in
  `packages/email` is `nok.toLocaleString("nb-NO")` with the locale's narrow
  space replaced by U+00A0, and any test or fixture that pastes the *output*
  ("1 200 kr") carries that character in the source file. A `sed`/python
  replace typed with an ASCII space silently matches nothing, and the test
  then fails on a string that looks identical on screen. Match around the
  amount, or capture the separator out of the file (`re.search(r'1(.)200')`)
...
| auth-boundaries.md | **the map a security review starts from** (verified 2026-09-11, open items re-checked 2026-09-25): the three callers (WorkOS session cookie, member capability tokens, Vipps HMAC), the middleware public allowlist + the worker's public-page cache rule (200 and no set-cookie, served to everyone, so a join page must never render visitor-dependent content), `requireOrgAccess` as the one org authorization, CSRF = Astro 7 `security.checkOrigin` default (curl POSTs need an `Origin:` header), the SQL layer is Drizzle-only (grep `sql.raw` / `prepare(` and stop), which db helpers are org-scoped vs id-only, the open items (login CSRF, receipt-by-agreement-id and the Vipps token cache key fixed 2026-09-25), the `origin/HEAD` fix the built-in `/security-review` needs in a worktree clone, **and what reaches Sentry**: `@sentry/cloudflare` attaches POST bodies + full URLs to events BY DEFAULT (worker.ts now disables body capture and runs `redactAlert` from `@stottemedlem/log`; a `redacted` tag on an event means the SDK started attaching again), and why a production 500 can never carry a stack (Astro 7 rethrows, adapter has no catch) |
| commit-and-pr-voice.md | **read before writing any commit message or PR body**: the subject is a declarative sentence about what is true of the product now (never `Add`/`Fix`/`feat:`), the body is prose not bullets and is expected to name what the change COST, and non-product work takes a scope prefix (`Skills:`, `render-card:`) |
...
| qr-codes.md | @stottemedlem/qr package split, the /bli-medlem/[slug]/qr embed contract + its former /api/qr address, the back office's shown card, the front-page card preview (marketing), qrcode-lib gotchas, apex routing, how much of a code a logo may cover (the heart in the middle) |
